...
class Factory(Protocol):
    def create_star(self, central_pressure: float, max_step: float) -> Any:
        """Creates a star."""


def create_stellar_family(
    fac: Factory, central_pressures: Iterable[float], max_steps: Iterable[float]
) -> pd.DataFrame:
    """Creates a list of stars/hybrid stars for a given EOS and
    array of central pressures."""
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(fac.create_star, cp, ms) for cp, ms in zip(central_pressures, max_steps)]
